...
public class DeskMySqlJdbcRepository implements DeskRepository {
    ...
    public Optional<List<DeskAvailabilityDTO>> findByRoomIncludeAvailability(String username, int roomId, String date, int offset, int limit) {
        ...
        // parameter 3 is int roomId
        // parameter 4 is int limit
        // parameter 5 is int offset
        // parameter 6 is String date

        String queryString = "SELECT desks.desk_id, desks.room_id, desks.desk_type_id, desk_type.desk_type_name, desks.desk_name, desks.notes, CASE WHEN booking_date IS NOT NULL THEN FALSE ELSE TRUE END AS available, CASE WHEN (SELECT COUNT(*) FROM booking where username = ? and booking_date = ?) > 0 THEN 1 ELSE 0 END AS does_user_have_booking_on_that_day FROM (SELECT * FROM desk WHERE room_id = ? LIMIT ? OFFSET ?) as desks LEFT JOIN (SELECT * FROM booking WHERE booking_date = ?) as bookings ON desks.desk_id = bookings.desk_id AND desks.room_id = bookings.room_id LEFT JOIN desk_type ON desks.desk_type_id = desk_type.desk_type_id ORDER BY desks.desk_id";


        try {
            return Optional.of(jdbc.query(queryString, new DeskAvailabilityRowMapper(), username, date, roomId, limit, offset, date));
        } catch (DataAccessException e) {
            e.printStackTrace();
        }
